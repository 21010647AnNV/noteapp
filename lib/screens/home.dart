...
class _HomeScreenState extends State<HomeScreen> {
  ...
  // void _updateEditedNoteInList(Note editedNote) async {
  //   await databaseHelper.update(editedNote); // Cập nhật ghi chú trong cơ sở dữ liệu

  //   setState(() {
  //     int index = filteredNotes.indexWhere((note) => note.id == editedNote.id);
  //     if (index != -1) {
  //       filteredNotes[index] = editedNote;
  //       _refreshNoteList(); // Cập nhật lại danh sách ghi chú sau khi chỉnh sửa
  //     }
  //   });
  // }
    ...
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton(
        onPressed: () async {
          ...
          if (newNote != null) {
            // await databaseHelper.create(newNote); // Thêm ghi chú mới vào cơ sở dữ liệu
